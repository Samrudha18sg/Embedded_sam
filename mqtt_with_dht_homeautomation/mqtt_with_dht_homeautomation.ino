...
#define relay2 D2
#include "DHT.h"
#define DHTPIN D6 //connect DHT data pin to D4
...
void loop() {
  ...
  while ((subscription = mqtt.readSubscription(2000))) {
    if (subscription == &onoffbutton) {
      ...
      digitalWrite(relay,onoffbutton_state);
...
//Adafruit_MQTT_Subscribe *subscription;
  ...
  while ((subscription = mqtt.readSubscription(2000))) {
   if (subscription == &onoffbutton2) {
      ...
      int onoffbutton2_state = atoi((char*)onoffbutton2.lastread);
      digitalWrite(relay2,onoffbutton2_state);
